...
getip() {
    ...
    # Get IP
    ip=$(curl --ipv4 -q -s -X POST -H 'content-type: application/json' \
        -d '{"execution_mode":"get"}' "$url" | grep -Eo '[0-9.]+')
...
if [ "$mode" = "get" ]; then
    ...
    # Call Lambda URL
    payload=$(printf '{"execution_mode":"%s","ddns_hostname":"%s","validation_hash":"%s"}' \
        "$mode" "$host" "$hash")
    curl --ipv4 -s -X POST -H 'content-type: application/json' -d "$payload" "$url"
else
    echo "Mode is required as 'get' or 'set', pass it with argument -m, i.e. $0 -m get -u https://xyz.lambda-url.eu-west1.on.aws/"
    exit 1
fi
